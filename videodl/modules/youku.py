...
class BaseVideoClient:
    ...
    def belongto(url, valid_domains=None):
        ...
        return any(host == d or host.endswith('.' + d) for d in (valid_domains or []))
    ...
    def get(self, url, **kwargs):
        ...
        for attempt in range(self.max_retries):
            try:
                return self.session.get(url, headers=headers, cookies=cookies, timeout=20, **kwargs)
            except Exception:
    ...
    def download(self, video_infos, num_threadings=1, request_overrides=None):
        ...
        for info in video_infos:
            if not info.with_valid_download_url:
                continue
            try:
                out = self._unique(info.save_path)
                info.save_path = str(out)
...
class YoukuVideoClient(BaseVideoClient):
    ...
    def __init__(self, **kwargs):
        ...
        if not self.default_parse_cookies:
            self.default_parse_cookies = {'__ysuid': self._getysuid(), 'xreferrer': 'http://www.youku.com'}
        self._initsession()
    ...
    def parsefromurl(self, url, request_overrides=None):
        info = VideoInfo(source=self.source, download_with_ffmpeg=True)
        overrides = dict(request_overrides or {})
        try:
            if not self.belongto(url):
                return []
            try:
                vid = parse_qs(urlparse(url).query, keep_blank_values=True)['vid'][0]
            except Exception:
                vid = urlparse(url).path.strip('/').split('/')[-1]
                vid = vid[:-5] if vid.endswith('.html') else vid
                vid = vid[3:] if vid.startswith('id_') else vid
            resp = self.get('https://log.mmstat.com/eg.js', **overrides)
            ...
            headers = dict(self.default_headers)
            headers['Referer'] = url
            resp = self.get('https://ups.youku.com/ups/get.json', params=params, headers=headers, **overrides)
            resp.raise_for_status()
            raw = resp.json()
            ...
            if not streams:
                raise RuntimeError('Youku did not return a playable m3u8 stream')
            title = legalize(safeget(data, ['video','title'], 'video'))
            info.download_url = streams[0][3]
            info.title = title
            info.identifier = vid
            info.cover_url = safeget(raw, ['data','video','logo']) or safeget(raw, ['data','preview','thumb_hd',0])
            info.raw_data = raw
            info.save_path = os.path.join(self.work_dir, self.source, f'{title}.mp4')
        except Exception as exc:
...
__all__ = ['BaseVideoClient', 'YoukuVideoClient', 'VideoInfo']
